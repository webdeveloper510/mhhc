...
class ProductManager {
	...
	public function ajax_install_product( array $payload ): array {
		...
		if ( ! $product ) {
			throw new Exception(
				strtr(
					esc_html_x( "Product with '[text_domain]' text domain not found.", 'Placeholders inside [] are not to be translated.', 'gk-gravityimport' ),
	...
	public function install_product( array $product ) {
		if ( ! file_exists( ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' ) ) {
			throw new Exception( esc_html__( 'Unable to load core WordPress files required to install the product.', 'gk-gravityimport' ) );
		}
		...
		if ( ! $product_download_link ) {
			throw new Exception( esc_html__( 'Unable to locate product download link.', 'gk-gravityimport' ) );
		}
		...
		try {
			...
			$error = join( ' ', [
				esc_html__( 'Installation failed.', 'gk-gravityimport' ),
				$e->getMessage()
			] );
		...
		if ( ! $installer->result ) {
			throw new Exception( esc_html__( 'Installation failed.', 'gk-gravityimport' ) );
		}
	}
	...
	public function update_product( array $product ) {
		...
		try {
			...
			$error = join( ' ', [
				esc_html__( 'Update failed.', 'gk-gravityimport' ),
	...
	public function ajax_delete_product( array $payload ): array {
		...
		if ( $product['active'] ) {
			throw new Exception(
				esc_html__( "Product must be deactivation before it can be deleted.", 'gk-gravityimport' )
			);
		}
	...
	public function delete_product( array $product ) {
		...
		if ( is_null( $result ) ) {
			throw new Exception( esc_html__( 'Could not delete the product due to missing filesystem credentials.', 'gk-gravityimport' ) );
		}
	}
	...
	public function activate_product( array $product ) {
		if ( $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Product is already active.', 'gk-gravityimport' ) );
		}
		...
		if ( is_wp_error( $result ) ) {
			throw new Exception(
				strtr(
					esc_html_x( 'Could not activate the product. [error]', 'Placeholders inside [] are not to be translated.', 'gk-gravityimport' ),
	...
	public function deactivate_product( array $product ) {
		if ( ! $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Product in not active.', 'gk-gravityimport' ) );
		}

		deactivate_plugins( $product['path'], false, CoreHelpers::is_network_admin() );

		if ( $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Could not deactivate the product.', 'gk-gravityimport' ) );
		}
	}
	...
	public function get_remote_products() {
		...
		if ( empty( $response ) || empty( $products ) ) {
			throw new Exception( esc_html__( 'Invalid product information received from the API.', 'gk-gravityimport' ) );
		}
	...
	public function truncate_product_changelog( $changelog, $product_url, $max_changelog_entries = 3, $link_to_full_changelog = true ) {
		...
		if ( count( $parsed_changelog[0] ) > count( $truncated_changelog ) && $link_to_full_changelog ) {
			$truncated_changelog[] = sprintf(
				...
				$product_url,
				esc_html__( 'View full changelog', 'gk-gravityimport' )
			);
		}
