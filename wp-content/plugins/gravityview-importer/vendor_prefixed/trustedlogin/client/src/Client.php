...
final class Client {
	...
	public function grant_access( $include_debug_data = false, $ticket_data = null ) {
		...
		if ( ! $this->config->meets_ssl_requirement() ) {
			return new \WP_Error( 'fails_ssl_requirement', esc_html__( 'TrustedLogin requires a secure connection using HTTPS.', 'gk-gravityimport' ) );
		}
	...
	public function revoke_access( $identifier = '' ) {
		...
		if ( ! empty( $should_be_deleted ) ) {
			...
			return new \WP_Error( 'support_user_not_deleted', esc_html__( 'The support user was not deleted.', 'gk-gravityimport' ) );
		}
