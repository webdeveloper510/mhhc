...
final class SecurityChecks {
	...
	public function verify( $passed_user_identifier = '' ) {
		...
		if ( $this->in_lockdown() ){
			...
			return new \WP_Error( 'in_lockdown', __( 'TrustedLogin temporarily disabled.' , 'gk-gravityimport') );
		}
		...
		if ( is_wp_error( $approved ) ){
			...
			$this->logging->log(
				sprintf(
					...
					__( 'There was an issue verifying the user identifier with TrustedLogin, aborting login. (%s)', 'gk-gravityimport' ),
