...
class SiteAccess {
	...
	public function sync_secret( $secret_id, $site_identifier_hash, $action = 'create' ) {
		...
		if ( ! in_array( $action, self::$sync_actions, true ) ) {
			return new \WP_Error( 'param_error', __( 'Unexpected action value', 'gk-gravityimport' ) );
		}
		...
		if ( empty( $response_json['success'] ) ) {
			return new \WP_Error( 'sync_error', __( 'Could not sync to TrustedLogin server', 'gk-gravityimport' ) );
		}
