 ...
 * Class SupportRole
 *
 * @package GravityKit\GravityImport\Foundation\ThirdParty\TrustedLogin\SupportRole
 *
...
final class SupportRole {
	...
	public function create( $new_role_slug = '', $clone_role_slug = '' ) {
		...
		$role_display_name = apply_filters( 'trustedlogin/' . $this->config->ns() . '/support_role/display_name',
			...
			sprintf( esc_html__( '%s Support', 'gk-gravityimport' ), $this->config->get_setting( 'vendor/title' ) ),
			$this
		);
