...
/**
 * gravityview-inline-edit - Sync User Registration (when using the GF User Registration Add-on)
 */
class GravityEdit_Entry_User_Registration {
	...
	public function add_hooks() {
		...
		if ( apply_filters( 'gk/gravityedit/user_registration/trigger_update', true ) ) {
			...
			add_filter( 'gk/gravityedit/entry-updated', array( $this, 'update_user' ), 10, 3 );
	...
	public function update_user( $update_result, $entry, $form_id ) {
		 ...
		 */
		$entry = apply_filters( 'gk/gravityedit/user_registration/entry', $entry, $form );
	...
	public function get_feed_configuration( $entry, $form ) {
		 ...
		 */
		$preserve_role = apply_filters( 'gk/gravityedit/user_registration/preserve_role', true, $config, $form, $entry );
		 ...
		 * @param array $entry Gravity Forms entry being edited
		 */
		$config = apply_filters( 'gk/gravityedit/user_registration/config', $config, $form, $entry );
	 ...
	 *
	 * @since 1.7.3
	 * @since 1.7.3 Returns false if user not found at $user_id
	 *
	...
	public function restore_display_name( $user_id = 0, $config = array(), $entry = array(), $password = '' ) {
		 ...
		 * @param array   $entry The Gravity Forms entry that was just updated
		 */
		$restored_user = apply_filters( 'gk/gravityedit/user_registration/restored_user', $restored_user, $this->_user_before_update, $user_after_update, $entry );
...
new GravityEdit_Entry_User_Registration();
