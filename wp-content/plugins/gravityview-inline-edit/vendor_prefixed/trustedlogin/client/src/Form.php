...
final class Form {
	...
	public function get_auth_screen() {
		...
		$content = array(
			...
			'secured_by_trustedlogin' => '<span class="trustedlogin-logo-medium"></span>' . esc_html__( 'Secured by TrustedLogin', 'gk-gravityedit' ),
	...
	private function get_terms_of_service_html() {
		 ...
		 */
		$tos_anchor = apply_filters( 'trustedlogin/' . $this->config->ns() . '/template/auth/terms_of_service/anchor', esc_html__( 'Terms of Service', 'gk-gravityedit' ) );
		...
		$variables = array(
			'ns'            => $this->config->ns(),
			'tos_text'      => esc_html__( 'By granting access, you agree to the {{tos_link}}.', 'gk-gravityedit' ),
	...
	private function get_details_html() {
		...
		if ( $this->is_create_ticket_enabled() ) {
			...
			$message_summary = sprintf(
				...
				esc_html__( 'Include a message for support?', 'gk-gravityedit' )
			);
			...
			', esc_html__( 'Please describe the issue you are having.', 'gk-gravityedit' ) );
		...
		if( $this->config->get_setting( 'clone_role' ) ) {
			...
			$roles_summary = sprintf( esc_html__( 'Create a user with a role based on %s.', 'gk-gravityedit' ), '<strong>' . $cloned_role . '</strong>' );

			if ( $this->config->get_setting( 'caps/add' ) || $this->config->get_setting( 'caps/remove' ) ) {
				$roles_summary .= sprintf( '<small class="tl-' . $ns . '-toggle" data-toggle=".tl-' . $ns . '-auth__role-container">%s <span class="dashicons dashicons--small dashicons-arrow-down-alt2"></span></small>', esc_html__( 'View modified role capabilities', 'gk-gravityedit' ) );
			}
	...
	private function get_debug_data_consent_html() {
		...
		$output = sprintf( '<h2><label><input type="checkbox" id="tl-{{ns}}-debug-data-consent" class="tl-{{ns}}-auth__checkbox--large" /> %s</label></h2>', strtr( esc_html__( 'Include the [link]Site Health[/link] troubleshooting report', 'gk-gravityedit' ), array(
	...
	private function get_admin_debug_html() {
		...
		$items = array(
			esc_html__( 'TrustedLogin Status', 'gk-gravityedit' ) => sprintf( '<a href="%s" target="_blank" rel="noopener noreferrer">%s</a>', 'https://status.trustedlogin.com', is_wp_error( wp_remote_request( 'https://app.trustedlogin.com/api/status' ) ) ? esc_html__( 'Offline', 'gk-gravityedit' ) : esc_html__( 'Online', 'gk-gravityedit' ) ),
			esc_html__( 'API Key', 'gk-gravityedit' ) => sprintf( '<code>%s</code>', $this->config->get_setting( 'auth/api_key' ) ),
			esc_html__( 'License Key', 'gk-gravityedit' ) => sprintf( '<code>%s</code>', $this->config->get_setting( 'auth/license_key' ) ),
			esc_html__( 'Log URL', 'gk-gravityedit' ) => sprintf( '<a href="%s" target="_blank" rel="noopener noreferrer">%s</a>', str_replace( ABSPATH, get_site_url() . '/', $this->logging->get_log_file_path() ), esc_html__( 'Download the log', 'gk-gravityedit' ) ),
			esc_html__( 'Log Level', 'gk-gravityedit' ) => $this->config->get_setting( 'logging/threshold', esc_html__( '(Default)', 'gk-gravityedit' ) ),
			esc_html__( 'Webhook URL', 'gk-gravityedit' ) => sprintf( '<code>%s</code>', $this->config->get_setting( 'webhook/url', '(Empty)' ) ),
			esc_html__( 'Vendor Public Key', 'gk-gravityedit' ) => sprintf( '<code>%s</code> (<a href="%s" target="_blank">%s</a>)', $encryption->get_vendor_public_key(), $encryption->get_remote_encryption_key_url(), esc_html__( 'Verify key', 'gk-gravityedit' ) ),
		);
		...
		return $this->prepare_output( $debugging_output, array(
			'ns' => $this->config->ns(),
			'debugging_label' => esc_html__( 'Debugging Info', 'gk-gravityedit' ),
			'debugging_info' => $debugging_info,
			'tl_config_label' => esc_html__( 'TrustedLogin Config', 'gk-gravityedit' ),
...
EOD;
			$access_key_output   = sprintf(
				...
				/* %2$s */
				esc_html__( 'Error', 'gk-gravityedit' ),
				/* %3$s */
				'div',
				/* %4$s */
				esc_html__( 'There was an error returning the access key.', 'gk-gravityedit' ),
				/* %5$s */
...
EOD;
			$access_key_output   = sprintf(
				...
				sanitize_title( $this->config->ns() ),
				/* %2$s */
				esc_html__( 'Site access key:', 'gk-gravityedit' ),
				/* %3$s */
				esc_html__( 'Access Key', 'gk-gravityedit' ),
				/* %4$s */
				esc_attr( $access_key ),
				/* %5$s */
				esc_html__( 'Copy', 'gk-gravityedit' ),
				/* %6$s */
				'div',
				/* %7$s */
				esc_html__( 'Copy the access key to your clipboard', 'gk-gravityedit' ),
				// translators: %s is the display name of the TrustedLogin support user.
				/* %8$s */
				sprintf( esc_html__( 'The access key is not a password; only %1$s will be able to access your site using this code. You may share this access key on support forums.', 'gk-gravityedit' ), $this->support_user->get_first()->display_name )
			);
		}
