...
use GravityKit\GravityMaps\Foundation\Licenses\WP\PluginsPage;
use GravityKit\GravityMaps\Foundation\Licenses\WP\UpdatesPage;
...
class Framework {
	...
	public function ajax_get_app_data( array $payload ) {
		...
		if ( ! $this->current_user_can( 'view_products' ) && ! $this->current_user_can( 'view_licenses' ) ) {
			throw new Exception( esc_html__( 'You do not have permission to view this page.', 'gk-gravitymaps' ) );
		}
	...
	public function get_framework_title() {
		...
		return esc_html__( 'Manage Your Kit', 'gk-gravitymaps' );
	}
