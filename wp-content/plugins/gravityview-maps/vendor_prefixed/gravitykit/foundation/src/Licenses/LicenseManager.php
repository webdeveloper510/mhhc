...
namespace GravityKit\GravityMaps\Foundation\Licenses;
...
class LicenseManager {
	...
	public function ajax_get_licenses_data( array $payload ) {
		if ( ! Framework::get_instance()->current_user_can( 'view_licenses' ) ) {
			throw new Exception( esc_html__( 'You do not have a permission to perform this action.', 'gk-gravitymaps' ) );
		}
	...
	public function get_license_key_status_message( $status ) {
		$statuses = [
			'site_inactive'       => esc_html__( 'The license key is valid, but it has not been activated for this site.', 'gk-gravitymaps' ),
			'inactive'            => esc_html__( 'The license key is valid, but it has not been activated for this site.', 'gk-gravitymaps' ),
			'no_activations_left' => esc_html__( 'This license has reached its activation limit.', 'gk-gravitymaps' ),
			'deactivated'         => esc_html__( 'This license has been deactivated.', 'gk-gravitymaps' ),
			'valid'               => esc_html__( 'This license key is valid and active.', 'gk-gravitymaps' ),
			'invalid'             => esc_html__( 'This license key is invalid.', 'gk-gravitymaps' ),
			'missing'             => esc_html__( 'This license key is invalid.', 'gk-gravitymaps' ),
			'revoked'             => esc_html__( 'This license key has been revoked.', 'gk-gravitymaps' ),
			'expired'             => esc_html__( 'This license key has expired.', 'gk-gravitymaps' ),
		];

		if ( empty( $statuses[ $status ] ) ) {
			LoggerFramework::get_instance()->warning( 'Unknown license status: ' . $status );

			return esc_html__( 'License status could not be determined.', 'gk-gravitymaps' );
		}
	...
	public function perform_remote_license_call( $license, $edd_action ) {
		...
		foreach ( (array) $response as $key => $data ) {
			if ( ! isset( $data['success'] ) || ! isset( $data['license'] ) || ! isset( $data['checksum'] ) ) {
				throw new Exception( esc_html__( 'License data received from the API is incomplete.', 'gk-gravitymaps' ) );
			}
	...
	public function ajax_activate_license( array $payload ) {
		...
		if ( empty( $payload['key'] ) ) {
			throw new Exception( esc_html__( 'Missing license key.', 'gk-gravitymaps' ) );
		}
	...
	public function activate_license( $license_key ) {
		...
		if ( isset( $licenses_data[ $license_key ] ) ) {
			throw new Exception( esc_html__( 'This license is already activated.', 'gk-gravitymaps' ) );
		}
	...
	public function ajax_deactivate_license( array $payload ) {
		...
		if ( empty( $licenses_data[ $license_key ] ) ) {
			throw new Exception( esc_html__( 'The license key is invalid.', 'gk-gravitymaps' ) );
		}
		...
		try {
			...
			if ( ! $force_removal && ! Arr::get( $response, '_raw.success' ) ) {
				...
				if ( ! Arr::get( $response, 'expiry' ) || ! $this->is_expired_license( Arr::get( $response, 'expiry' ) ) ) {
					throw new Exception( esc_html__( 'Failed to deactivate license.', 'gk-gravitymaps' ) );
				}
			}
		} catch ( Exception $e ) {
			if ( ! $force_removal ) {
	...
	public function modify_license_data_for_frontend_output( $license ) {
		...
		if ( preg_match( '/[^a-z]/i', $expiry ) ) {
			...
			$expiry = $expired
				? human_time_diff( $expiry, current_time( 'timestamp' ) ) . ' ' . esc_html_x( 'ago', 'Indicates "time ago"', 'gk-gravitymaps' )
