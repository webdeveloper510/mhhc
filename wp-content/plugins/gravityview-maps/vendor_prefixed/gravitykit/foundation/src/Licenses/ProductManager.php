...
use GravityKit\GravityMaps\Foundation\Licenses\WP\WPUpgraderSkin;
use Plugin_Upgrader;
...
class ProductManager {
	...
	public function ajax_install_product( array $payload ): array {
		...
		if ( ! $product ) {
			throw new Exception(
				strtr(
					esc_html_x( "Product with '[text_domain]' text domain not found.", 'Placeholders inside [] are not to be translated.', 'gk-gravitymaps' ),
	...
	public function install_product( array $product ) {
		if ( ! file_exists( ABSPATH . 'wp-admin/includes/class-wp-upgrader.php' ) ) {
			throw new Exception( esc_html__( 'Unable to load core WordPress files required to install the product.', 'gk-gravitymaps' ) );
		}
		...
		if ( ! $product_download_link ) {
			throw new Exception( esc_html__( 'Unable to locate product download link.', 'gk-gravitymaps' ) );
		}
		...
		try {
			...
			$error = join( ' ', [
				esc_html__( 'Installation failed.', 'gk-gravitymaps' ),
				$e->getMessage()
			] );
		...
		if ( ! $installer->result ) {
			throw new Exception( esc_html__( 'Installation failed.', 'gk-gravitymaps' ) );
		}
	}
	...
	public function update_product( array $product ) {
		...
		try {
			...
			$error = join( ' ', [
				esc_html__( 'Update failed.', 'gk-gravitymaps' ),
	...
	public function ajax_delete_product( array $payload ): array {
		...
		if ( $product['active'] ) {
			throw new Exception(
				esc_html__( "Product must be deactivation before it can be deleted.", 'gk-gravitymaps' )
			);
		}
	...
	public function delete_product( array $product ) {
		...
		if ( is_null( $result ) ) {
			throw new Exception( esc_html__( 'Could not delete the product due to missing filesystem credentials.', 'gk-gravitymaps' ) );
		}
	}
	...
	public function activate_product( array $product ) {
		if ( $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Product is already active.', 'gk-gravitymaps' ) );
		}
		...
		if ( is_wp_error( $result ) ) {
			throw new Exception(
				strtr(
					esc_html_x( 'Could not activate the product. [error]', 'Placeholders inside [] are not to be translated.', 'gk-gravitymaps' ),
	...
	public function deactivate_product( array $product ) {
		if ( ! $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Product in not active.', 'gk-gravitymaps' ) );
		}

		deactivate_plugins( $product['path'], false, CoreHelpers::is_network_admin() );

		if ( $this->is_product_active_in_current_context( $product['path'] ) ) {
			throw new Exception( esc_html__( 'Could not deactivate the product.', 'gk-gravitymaps' ) );
		}
	}
	...
	public function get_remote_products() {
		...
		if ( empty( $response ) || empty( $products ) ) {
			throw new Exception( esc_html__( 'Invalid product information received from the API.', 'gk-gravitymaps' ) );
		}
	...
	public function truncate_product_changelog( $changelog, $product_url, $max_changelog_entries = 3, $link_to_full_changelog = true ) {
		...
		if ( count( $parsed_changelog[0] ) > count( $truncated_changelog ) && $link_to_full_changelog ) {
			$truncated_changelog[] = sprintf(
				...
				$product_url,
				esc_html__( 'View full changelog', 'gk-gravitymaps' )
			);
		}
