	...
	public function modify_display_of_updates_table() {
		...
		$product_html_markup = array_map( function ( $product ) {
			$product_logo_text = strtr(
				esc_html__( '[product] logo', 'gk-gravitymaps' ),
				[
			...
			$update_description_text = strtr(
				esc_html_x( 'You have version [current_version] installed. Before updating to [new_version], please [link]review the requirements[/link].', 'Placeholders inside [] are not to be translated.', 'gk-gravitymaps' ),
				[
...
JS;
	...
	public function prevent_update( $response, $args ) {
		...
		return new WP_Error( 'gk_product_unmet_dependency', strtr(
			esc_html_x( 'This product has unmet dependencies. [link]Review the requirements[/link] and try updating again.', 'Placeholders inside [] are not to be translated.', 'gk-gravitymaps' ),
			[
