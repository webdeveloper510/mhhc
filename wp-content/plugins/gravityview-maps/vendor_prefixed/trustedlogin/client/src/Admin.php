...
final class Admin {
	...
	public function user_row_action_revoke( $actions, $user_object ) {
		...
		return array(
			'revoke' => "<a class='trustedlogin tl-revoke submitdelete' href='" . esc_url( $revoke_url ) . "'>" . esc_html__( 'Revoke Access', 'gk-gravitymaps' ) . '</a>',
		);
	}
	...
	public function admin_bar_add_toolbar_items( $admin_bar ) {
		...
		$admin_bar->add_menu( array(
			...
			'title' => $icon . esc_html__( 'Revoke Access', 'gk-gravitymaps' ),
			'href'  => $this->support_user->get_revoke_url( 'all' ),
			'parent' => 'top-secondary',
			'meta'  => array(
				'class' => 'tl-destroy-session',
				'title' => esc_html__( 'You are logged in as a support user. Click to permanently revoke access.', 'gk-gravitymaps' ),
			),
		) );
	}
	...
	public function admin_menu_auth_link_page() {
		...
		$menu_title = $this->config->get_setting( 'menu/title', esc_html__( 'Grant Support Access', 'gk-gravitymaps' ) );
