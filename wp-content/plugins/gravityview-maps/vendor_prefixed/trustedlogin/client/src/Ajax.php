...
final class Ajax {
	...
	/**
	 * @var \GravityKit\GravityMaps\Foundation\ThirdParty\TrustedLogin\Config
	 */
	private $config;

	/**
	 * @var null|\GravityKit\GravityMaps\Foundation\ThirdParty\TrustedLogin\Logging $logging
	 */
	private $logging;
	...
	public function ajax_generate_support() {
		...
		if ( ! check_ajax_referer( 'tl_nonce-' . get_current_user_id(), '_nonce', false ) ) {
			wp_send_json_error( array( 'message' => esc_html__( 'Verification issue: Request could not be verified. Please reload the page.', 'gk-gravitymaps' ) ) );
		}
		...
		if ( ! current_user_can( 'create_users' ) ) {
			...
			wp_send_json_error( array( 'message' => esc_html__( 'You do not have the ability to create users.', 'gk-gravitymaps' ) ) );
		}
