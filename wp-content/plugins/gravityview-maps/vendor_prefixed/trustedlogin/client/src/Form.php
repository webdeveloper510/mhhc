 ...
 * Class Form
 *
 * @package GravityKit\GravityMaps\Foundation\ThirdParty\TrustedLogin\Client
 *
 * @copyright 2021 Katz Web Services, Inc.
 */

namespace GravityKit\GravityMaps\Foundation\ThirdParty\TrustedLogin;
...
final class Form {
	...
	public function get_auth_header_html() {
		...
		$unknown_user_text = sprintf( esc_html__( 'Unknown (User #%d)', 'gk-gravitymaps' ), $_user_creator_id );
		...
		$content = array(
			...
			'revoke_access_button' => sprintf( '<a href="%s" class="button button-danger alignright tl-client-revoke-button">%s</a>', $revoke_url, esc_html__( 'Revoke Access', 'gk-gravitymaps' ) ),
			// translators: %s is the display name of the user who granted access
			'auth_meta'            => sprintf( esc_html__( 'Created %s ago by %s', 'gk-gravitymaps' ), human_time_diff( strtotime( $support_user->user_registered ) ), $auth_meta ),
		);
	...
	public function get_auth_screen() {
		...
		$content = array(
			...
			'secured_by_trustedlogin' => '<span class="trustedlogin-logo-medium"></span>' . esc_html__( 'Secured by TrustedLogin', 'gk-gravitymaps' ),
	...
	private function get_terms_of_service_html() {
		 ...
		 */
		$tos_anchor = apply_filters( 'trustedlogin/' . $this->config->ns() . '/template/auth/terms_of_service/anchor', esc_html__( 'Terms of Service', 'gk-gravitymaps' ) );
		...
		$variables = array(
			'ns'            => $this->config->ns(),
			'tos_text'      => esc_html__( 'By granting access, you agree to the {{tos_link}}.', 'gk-gravitymaps' ),
	...
	private function get_reference_html() {
		...
		$content = array(
			...
			'reference_text' => sprintf( esc_html__( 'Reference #%s', 'gk-gravitymaps' ), $reference_id ),
			'ns'             => $this->config->ns(),
	...
	private function get_intro() {
		...
		if ( $has_access ) {
			foreach ( $has_access as $access ) {
				...
				$intro = sprintf( esc_html__( '%1$s has site access that expires in %2$s.', 'gk-gravitymaps' ), '<a href="' . esc_url( $this->config->get_setting( 'vendor/website' ) ) . '" target="_blank" rel="noopener noreferrer">' . $this->config->get_setting( 'vendor/title' ) . '</a>', str_replace( ' ', '&nbsp;', $this->support_user->get_expiration( $access, true, false ) ) );
			}
		...
		if ( $this->is_login_screen() ) {
			...
			$intro = sprintf( esc_html__( '%1$s would like support access to this site.', 'gk-gravitymaps' ), '<a href="' . esc_url( $this->config->get_setting( 'vendor/website' ) ) . '">' . $this->config->get_display_name() . '</a>' );
		} else {
			// translators: %1$s is replaced with the name of the software developer (e.g. "Acme Widgets")
			$intro = sprintf( esc_html__( 'Grant %1$s access to this site.', 'gk-gravitymaps' ), '<a href="' . esc_url( $this->config->get_setting( 'vendor/website' ) ) . '">' . $this->config->get_display_name() . '</a>' );
		}
	...
	private function get_details_html() {
		...
		if ( $this->is_create_ticket_enabled() ) {
			...
			$message_summary = sprintf(
				...
				esc_html__( 'Include a message for support?', 'gk-gravitymaps' )
			);
			...
			', esc_html__( 'Please describe the issue you are having.', 'gk-gravitymaps' ) );
		...
		$expire_summary = sprintf( esc_html__( 'Access this site for %s.', 'gk-gravitymaps' ), '<strong>' . human_time_diff( 0, $this->config->get_setting( 'decay' ) ) . '</strong>' );

		// translators: %s is replaced by the amount of time that the login will be active for (e.g. "1 week")
		$expire_desc = '<small>' . sprintf( esc_html__( 'Access auto-expires in %s. You may revoke access at any time.', 'gk-gravitymaps' ), human_time_diff( 0, $this->config->get_setting( 'decay' ) ) ) . '</small>';
		...
		if( $this->config->get_setting( 'clone_role' ) ) {
			...
			$roles_summary = sprintf( esc_html__( 'Create a user with a role based on %s.', 'gk-gravitymaps' ), '<strong>' . $cloned_role . '</strong>' );

			if ( $this->config->get_setting( 'caps/add' ) || $this->config->get_setting( 'caps/remove' ) ) {
				$roles_summary .= sprintf( '<small class="tl-' . $ns . '-toggle" data-toggle=".tl-' . $ns . '-auth__role-container">%s <span class="dashicons dashicons--small dashicons-arrow-down-alt2"></span></small>', esc_html__( 'View modified role capabilities', 'gk-gravitymaps' ) );
			}

		} else {

			// translators: %s is replaced with the name of the role (e.g. "Administrator")
			$roles_summary = sprintf( esc_html__( 'Create a user with a role of %s.', 'gk-gravitymaps' ), '<strong>' . $cloned_role . '</strong>' );
		}
	...
	private function get_debug_data_consent_html() {
		...
		$output = sprintf( '<h2><label><input type="checkbox" id="tl-{{ns}}-debug-data-consent" class="tl-{{ns}}-auth__checkbox--large" /> %s</label></h2>', strtr( esc_html__( 'Include the [link]Site Health[/link] troubleshooting report', 'gk-gravitymaps' ), array(
	...
	private function get_caps_html() {
		...
		$caps = '';
		$caps .= $this->get_caps_section( $added, __( 'Additional capabilities:', 'gk-gravitymaps' ), 'dashicons-yes-alt' );
		$caps .= $this->get_caps_section( $removed, __( 'Removed capabilities:', 'gk-gravitymaps' ), 'dashicons-dismiss' );
	...
	private function get_notices_html() {
		...
		$content = array(
			...
			'local_site'            => sprintf( esc_html__( '%s support may not be able to access this site.', 'gk-gravitymaps' ), $this->config->get_setting( 'vendor/title' ) ),
			'need_access'           => esc_html__( 'This website is running in a local development environment. To provide support, we must be able to access your site using a publicly-accessible URL.', 'gk-gravitymaps' ),
			'about_live_access_url' => esc_url( $this->config->get_setting( 'vendor/about_live_access_url', self::ABOUT_LIVE_ACCESS_URL ) ),
			'learn_more'            => esc_html__( 'Learn more.', 'gk-gravitymaps' ),
		);
	...
	private function get_footer_html() {
		...
		$footer_links = array(
			esc_html__( 'Learn about TrustedLogin', 'gk-gravitymaps' )                    => self::ABOUT_TL_URL,
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
	...
	private function get_admin_debug_html() {
		...
		$items = array(
			esc_html__( 'TrustedLogin Status', 'gk-gravitymaps' ) => sprintf( '<a href="%s" target="_blank" rel="noopener noreferrer">%s</a>', 'https://status.trustedlogin.com', is_wp_error( wp_remote_request( 'https://app.trustedlogin.com/api/status' ) ) ? esc_html__( 'Offline', 'gk-gravitymaps' ) : esc_html__( 'Online', 'gk-gravitymaps' ) ),
			esc_html__( 'API Key', 'gk-gravitymaps' ) => sprintf( '<code>%s</code>', $this->config->get_setting( 'auth/api_key' ) ),
			esc_html__( 'License Key', 'gk-gravitymaps' ) => sprintf( '<code>%s</code>', $this->config->get_setting( 'auth/license_key' ) ),
			esc_html__( 'Log URL', 'gk-gravitymaps' ) => sprintf( '<a href="%s" target="_blank" rel="noopener noreferrer">%s</a>', str_replace( ABSPATH, get_site_url() . '/', $this->logging->get_log_file_path() ), esc_html__( 'Download the log', 'gk-gravitymaps' ) ),
			esc_html__( 'Log Level', 'gk-gravitymaps' ) => $this->config->get_setting( 'logging/threshold', esc_html__( '(Default)', 'gk-gravitymaps' ) ),
			esc_html__( 'Webhook URL', 'gk-gravitymaps' ) => sprintf( '<code>%s</code>', $this->config->get_setting( 'webhook/url', '(Empty)' ) ),
			esc_html__( 'Vendor Public Key', 'gk-gravitymaps' ) => sprintf( '<code>%s</code> (<a href="%s" target="_blank">%s</a>)', $encryption->get_vendor_public_key(), $encryption->get_remote_encryption_key_url(), esc_html__( 'Verify key', 'gk-gravitymaps' ) ),
		);
		...
		return $this->prepare_output( $debugging_output, array(
			'ns' => $this->config->ns(),
			'debugging_label' => esc_html__( 'Debugging Info', 'gk-gravitymaps' ),
			'debugging_info' => $debugging_info,
			'tl_config_label' => esc_html__( 'TrustedLogin Config', 'gk-gravitymaps' ),
	...
	public function get_button( $atts = array() ) {
		...
		$defaults = array(
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
			'text'        => sprintf( esc_html__( 'Grant %s Access', 'gk-gravitymaps' ), $this->config->get_display_name() ),
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
			'exists_text' => sprintf( esc_html__( 'Extend %s Access', 'gk-gravitymaps' ), $this->config->get_display_name(), ucwords( human_time_diff( time(), time() + $this->config->get_setting( 'decay' ) ) ) ),
			'size'        => 'hero',
		...
		if ( $atts['powered_by'] ) {
			$powered_by = sprintf(
				...
				esc_html__( 'Secured by TrustedLogin', 'gk-gravitymaps' )
			);
		}
	...
	public function translations() {
		...
		$query_args = apply_filters(
			...
			array(
				'message' => __( 'Could not create TrustedLogin access.', 'gk-gravitymaps' ),
		...
		$error_content = sprintf(
			...
			sprintf(
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
				esc_html__( 'The user details could not be sent to %1$s automatically.', 'gk-gravitymaps' ),
				$vendor_title
			),
			sprintf(
			// translators: %1$s is the vendor support url and %2$s is the vendor title
				__( 'Please <a href="%1$s" target="_blank">click here</a> to go to the %2$s support site', 'gk-gravitymaps' ),
		...
		$translations = array(
			'buttons' => array(
				'confirm'    => esc_html__( 'Confirm', 'gk-gravitymaps' ),
				'ok'         => esc_html__( 'Ok', 'gk-gravitymaps' ),
				// translators: %1$s is the vendor title
				'go_to_site' => sprintf( __( 'Go to %1$s support site', 'gk-gravitymaps' ), $vendor_title ),
				'close'      => esc_html__( 'Close', 'gk-gravitymaps' ),
				'cancel'     => esc_html__( 'Cancel', 'gk-gravitymaps' ),
				// translators: %1$s is the vendor title
				'revoke'     => sprintf( esc_html__( 'Revoke %1$s support access', 'gk-gravitymaps' ), $vendor_title ),
				'copy'       => esc_html__( 'Copy', 'gk-gravitymaps' ),
				'copied'     => esc_html__( 'Copied!', 'gk-gravitymaps' ),
			),
			'a11y'    => array(
				'opens_new_window' => esc_attr__( '(This link opens in a new window.)', 'gk-gravitymaps' ),
				'copied_text'      => esc_html__( 'The access key has been copied to your clipboard.', 'gk-gravitymaps' ),
			),
			'status'  => array(
				'synced'             => array(
					'title'   => esc_html__( 'Support access granted', 'gk-gravitymaps' ),
					'content' => sprintf(
					// translators: %1$s is the vendor title
						__( 'A temporary support user has been created, and sent to %1$s support.', 'gk-gravitymaps' ),
						$vendor_title
					),
				),
				'pending'            => array(
					// translators: %1$s is the vendor title
					'content' => sprintf( __( 'Generating & encrypting secure support access for %1$s', 'gk-gravitymaps' ), $vendor_title ),
				),
				'extending'          => array(
					// translators: %1$s is the vendor title and %2$s is the human-readable expiration time (for example, "1 week")
					'content' => sprintf( __( 'Extending support access for %1$s by %2$s', 'gk-gravitymaps' ), $vendor_title, human_time_diff( time(), time() + $this->config->get_setting( 'decay' ) ) ),
				),
				'syncing'            => array(
					// translators: %1$s is the vendor title
					'content' => sprintf( __( 'Sending encrypted access to %1$s.', 'gk-gravitymaps' ), $vendor_title ),
				),
				'error'              => array(
					// translators: %1$s is the vendor title
					'title'   => sprintf( __( 'Error syncing support user to %1$s', 'gk-gravitymaps' ), $vendor_title ),
				...
				'cancel'             => array(
					'title'   => esc_html__( 'Action Cancelled', 'gk-gravitymaps' ),
					'content' => sprintf(
					// translators: %1$s is the vendor title
						__( 'A support account for %1$s was not created.', 'gk-gravitymaps' ),
						$vendor_title
					),
				),
				'failed'             => array(
					'title'   => esc_html__( 'Support Access Was Not Granted', 'gk-gravitymaps' ),
					'content' => esc_html__( 'There was an error granting access: ', 'gk-gravitymaps' ),
				),
				'failed_permissions' => array(
					'content' => esc_html__( 'Your authorized session has expired. Please refresh the page.', 'gk-gravitymaps' ),
				),
				'accesskey'          => array(
					'title'       => esc_html__( 'TrustedLogin Key Created', 'gk-gravitymaps' ),
					'content'     => sprintf(
					// translators: %1$s is the vendor title
						__( 'Share this TrustedLogin Key with %1$s to give them secure access:', 'gk-gravitymaps' ),
						$vendor_title
					),
					'revoke_link' => esc_url( add_query_arg( array( Endpoint::REVOKE_SUPPORT_QUERY_PARAM => $this->config->ns() ), admin_url() ) ),
				),
				'error404'           => array(
					'title'   => esc_html__( 'The TrustedLogin vendor could not be found.', 'gk-gravitymaps' ),
					'content' => '',
				),
				'error409'           => array(
					'title'   => sprintf(
					// translators: %1$s is the vendor title
						__( '%1$s Support user already exists', 'gk-gravitymaps' ),
						$vendor_title
					),
					'content' => sprintf(
						wp_kses(
						// translators: %1$s is the vendor title, %2$s is the URL to the users list page
							__( 'A support user for %1$s already exists. You may revoke this support access from your <a href="%2$s" target="_blank">Users list</a>.', 'gk-gravitymaps' ),
	...
	public function output_support_users( $print = true, $atts = array() ) {
		...
		if ( empty( $support_users ) ) {
			...
			// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
			$return = '<h3>' . sprintf( esc_html__( 'No %s users exist.', 'gk-gravitymaps' ), $this->config->get_setting( 'vendor/title' ) ) . '</h3>';
...
EOD;
			$access_key_output   = sprintf(
				...
				/* %2$s */
				esc_html__( 'Error', 'gk-gravitymaps' ),
				/* %3$s */
				'div',
				/* %4$s */
				esc_html__( 'There was an error returning the access key.', 'gk-gravitymaps' ),
				/* %5$s */
...
EOD;
			$access_key_output   = sprintf(
				...
				sanitize_title( $this->config->ns() ),
				/* %2$s */
				esc_html__( 'Site access key:', 'gk-gravitymaps' ),
				/* %3$s */
				esc_html__( 'Access Key', 'gk-gravitymaps' ),
				/* %4$s */
				esc_attr( $access_key ),
				/* %5$s */
				esc_html__( 'Copy', 'gk-gravitymaps' ),
				/* %6$s */
				'div',
				/* %7$s */
				esc_html__( 'Copy the access key to your clipboard', 'gk-gravitymaps' ),
				// translators: %s is the display name of the TrustedLogin support user.
				/* %8$s */
				sprintf( esc_html__( 'The access key is not a password; only %1$s will be able to access your site using this code. You may share this access key on support forums.', 'gk-gravitymaps' ), $this->support_user->get_first()->display_name )
			);
		}
	...
	public function admin_notice_revoked() {
		...
		<div class="notice notice-success is-dismissible">
			<h3><?php
				// translators: %s is replaced with the name of the software developer (e.g. "Acme Widgets")
				echo esc_html( sprintf( __( '%s access revoked.', 'gk-gravitymaps' ), $this->config->get_setting( 'vendor/title' ) ) );
				?></h3>
			<?php if ( ! current_user_can( 'delete_users' ) ) { ?>
				<p><?php echo esc_html__( 'You may safely close this window.', 'gk-gravitymaps' ); ?></p>
			<?php } ?>
		</div>
		<?php
