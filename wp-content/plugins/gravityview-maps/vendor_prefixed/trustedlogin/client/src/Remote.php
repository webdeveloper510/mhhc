...
final class Remote {
	...
	static public function check_response_code( $api_response ) {
		...
		switch ( $response_code ) {
			...
			case 400:
			case 423:
				return new \WP_Error( 'unable_to_verify', esc_html__( 'Unable to verify Pause Mode.', 'gk-gravitymaps' ), $api_response );

			case 401:
				return new \WP_Error( 'unauthenticated', esc_html__( 'Authentication failed.', 'gk-gravitymaps' ), $api_response );

			case 402:
				return new \WP_Error( 'account_error', esc_html__( 'TrustedLogin account issue.', 'gk-gravitymaps' ), $api_response );

			case 403:
				return new \WP_Error( 'invalid_token', esc_html__( 'Invalid tokens.', 'gk-gravitymaps' ), $api_response );

			// the KV store was not found, possible issue with endpoint
			case 404:
				return new \WP_Error( 'not_found', esc_html__( 'The TrustedLogin vendor was not found.', 'gk-gravitymaps' ), $api_response );
			...
			case 'http_request_failed':
				return new \WP_Error( 'unavailable', esc_html__( 'The TrustedLogin site is not currently online.', 'gk-gravitymaps' ), $api_response );

			// Server error
			case 501:
			case 502:
			case 522:
				return new \WP_Error( 'server_error', esc_html__( 'The TrustedLogin site is not currently available.', 'gk-gravitymaps' ), $api_response );

			// wp_remote_retrieve_response_code() couldn't parse the $api_response
			case '':
				return new \WP_Error( 'invalid_response', esc_html__( 'Invalid response.', 'gk-gravitymaps' ), $api_response );
	...
	public function handle_response( $api_response, $required_keys = array() ) {
		...
		if ( empty( $response_body ) ) {
			...
			return new \WP_Error( 'missing_response_body', esc_html__( 'The response was invalid.', 'gk-gravitymaps' ), $api_response );
		}
		...
		foreach ( (array) $required_keys as $required_key ) {
			if ( ! isset( $response_json[ $required_key ] ) ) {
				...
				return new \WP_Error( 'missing_required_key', sprintf( esc_html__( 'Invalid response. Missing key: %s', 'gk-gravitymaps' ), $required_key ), $response_body );
			}
		}
