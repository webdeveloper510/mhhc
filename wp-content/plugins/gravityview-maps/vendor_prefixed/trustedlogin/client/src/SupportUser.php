...
final class SupportUser {
	...
	public function create() {
		...
		if ( $user_id_of_email ) {
			...
			if ( ! $allow_existing_user_match ) {
				return new \WP_Error( 'email_exists', esc_html__( 'User not created; User with that email already exists', 'gk-gravitymaps' ) );
			}
	...
	private function generate_unique_username() {
		...
		$username = sprintf( esc_html__( '%s Support', 'gk-gravitymaps' ), $this->config->get_setting( 'vendor/title' ) );
